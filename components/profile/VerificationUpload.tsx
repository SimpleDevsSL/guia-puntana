...
import { Upload, Loader2, Clock, ShieldCheck, AlertCircle } from 'lucide-react';
...
export default function VerificationUpload({
  ...
  useEffect(() => {
    const checkStatus = async () => {
      ...
      if (insignias.includes('Usuario Verificado')) {
      ...
      <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
        <div className="flex-1">
          ...
          <p className="mb-4 text-sm leading-relaxed text-gray-500 dark:text-gray-400">
            Sube una foto de tu DNI para obtener la insignia de{' '}
            <span className="font-bold text-gray-900 dark:text-white">
              Usuario Verificado
            </span>
            . Esto aumenta drásticamente la confianza de tus clientes
            potenciales.
          </p>
          ...
          {verificationStatus === 'rejected' && (
            <div className="mb-4 flex items-center gap-2 rounded-lg bg-red-50 p-3 text-sm text-red-600 dark:bg-red-900/20 dark:text-red-300">
              <AlertCircle size={16} />
              Tu solicitud anterior fue rechazada. Por favor sube una foto más
              clara.
            </div>
          )}
        </div>
        ...
        <label
          className={`group relative flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 px-8 py-6 transition-all hover:border-orange-400 hover:bg-orange-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:border-orange-500/50 dark:hover:bg-orange-900/10 ${uploading ? 'pointer-events-none opacity-70' : ''} `}
        >
          <input
            type="file"
            accept="image/*"
          ...
          {uploading ? (
            ...
            <Upload
              className="text-gray-400 transition-colors group-hover:text-orange-600"
              size={28}
            />
          )}

          <span className="text-sm font-bold text-gray-600 group-hover:text-orange-700 dark:text-gray-300 dark:group-hover:text-orange-400">
