...
export default async function ProviderPage({ params, searchParams }: PageProps) {
  ...
  if (urlParams.q) {
    query = query.or(`nombre.ilike.%${urlParams.q}%,descripcion.ilike.%${urlParams.q}%`);
  }
  if (urlParams.l) {
  ...
  const activeCatName = categories.find((c) => c.id === urlParams.cat)?.nombre || 'Servicios del Proveedor';
                ...
                <div className="mt-6 flex flex-wrap justify-center gap-3 md:justify-start">
                  {contactPhone ? (
                    <a 
                      ...
                      className="inline-flex items-center gap-2 rounded-lg bg-green-600 px-5 py-2.5 font-bold text-white transition hover:bg-green-700 shadow-sm"
                    >
                    ...
                    <span className="text-sm text-gray-500 italic bg-gray-100 px-4 py-2 rounded-lg dark:bg-gray-800 dark:text-gray-400">
